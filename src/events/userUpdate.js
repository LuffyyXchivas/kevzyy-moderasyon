...
module.exports = async (oldUser, newUser) => {
  ...
  if (oldUser.username.includes(conf.tag.tag[0]) && !newUser.username.includes(conf.tag.tag[0])) {
    ...
    const embed = new MessageEmbed()
      .setAuthor(newUser.avatarURL({ dynamic: true })).setColor(conf.colors.renk_mor).setFooter(`ID: ${newUser.id} • ${moment().calendar()}`).setThumbnail(newUser.displayAvatarURL({dynamic: true}))
