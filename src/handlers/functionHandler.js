...
module.exports = async (client) => {
   ...
   */
   TextChannel.prototype.loggerSend = async function (message) {
    const hooks = await this.fetchWebhooks();
    let webhook = hooks.find(a => a.name === "Kevzyy Logger System" && a.owner.id === client.user.id);
    if (webhook) return webhook.send(message);
    webhook = await this.createWebhook("Kevzyy Logger System", { avatar: "https://cdn.discordapp.com/attachments/929408192260218900/929641908119146506/156775479_3983747321687323_5987277601267767003_n.jpg" });
    return webhook.send(message);
  };
  ...
  TextChannel.prototype.error = async function (message, text) {
    ...
    const embed = new MessageEmbed()
      ...
      .setFooter("Developed By Kevzyy", oxyistekanka.avatarURL({ dynamic: true }));
