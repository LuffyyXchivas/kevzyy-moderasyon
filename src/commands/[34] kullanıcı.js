...
module.exports = {
   ...
   ${emojis.yildiz} ${member.user.toString()} (${member.roles.highest}) kişisinin sunucu verileri
    **───────────────**
    **${emojis.ping2} Ses Bilgileri:**
  ...
  • Diğer: \`${await category(filteredParents.map(x => x.id))}\`
    **───────────────**
    **${emojis.ping} Sesli Kanal Bilgileri: (\`Toplam ${voiceLength} kanal\`)**
    ${voiceTop}
    **───────────────**
    **${emojis.ping} Mesaj Bilgileri: (\`Toplam ${messageData ? messageData.topStat : 0} mesaj\`)**
    ${messageTop}
    **───────────────**
    ${coinStatus}
    `);
		embed.addField(`${emojis.star} Ses Verileri:`, `
     \`•\` Haftalık Ses: \`${voiceWeekly}\`
     \`•\` Günlük Ses: \`${voiceDaily}\`
    `, true);
		embed.addField(`${emojis.star} Mesaj Verileri:`, `
