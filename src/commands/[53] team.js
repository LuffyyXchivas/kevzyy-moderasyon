...
${emojis.ates} Ses kanallarında bulunan üye sayısı: \`${message.guild.members.cache.filter(x => x.roles.cache.has(team) && x.voice.channel).size}\`
...
\`•\` Sesteki üye sayısı: ${message.guild.members.cache.filter(x => x.roles.cache.has(team) && x.voice.channel).size}
      `).join("\n"));
      message.channel.send(embed);
    } else {
      embed.setThumbnail(message.guild.iconURL({ dynamic: true }));
