const { tik, carp } = require("../configs/emojis.json");
...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    await question.react(tik);
    await question.react(carp);
    const answer = await question.awaitReactions((reaction, user) => [tik, carp].includes(reaction.emoji.toString()) && user.id === member.user.id, { max: 1, time: 60000, errors: ["time"] }).catch(() => { question.edit(embed.setDescription("İşlem iptal edildi!")) });
    if (answer.first().emoji.toString() === tik) {
