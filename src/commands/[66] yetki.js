...
module.exports = {
  conf: {
    aliases: ["yetki"],
    name: "yetki",
    help: "yetki [ekle] [coin] [rol(ler)] / [sil] [coin] / [temizle] / [liste]",
  ...
  run: async (client, message, args, embed) => {
    ...
    if (["ekle", "add"].includes(args[0])) {
      ...
      message.channel.send(embed.setDescription("Tüm yetkiler başarıyla temizlendi!"));
    } else if (["liste", "list"].includes(args[0])) message.channel.send(embed.setDescription(global.rankdb.get("ranks").sort((a, b) => a.coin - b.coin).map((x) => `${Array.isArray(x.role) ? x.role.map(x => `<@&${x}>`).join(", ") : `<@&${x.role}>`}: ${x.coin}`)));
  }
};
