...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    data = data.map((x) => `${emojis.info} #${x.id} ${x.active ? emojis.mark : emojis.cross} **[${x.type}]** ${moment(x.date).format("LLL")} tarihinde, <@${x.staff}> tarafından, \`${x.reason}\` nedeniyle, ${x.type.toLowerCase().replace("-", " ")} cezası almış.`).join("\n");
